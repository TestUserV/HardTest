...
public class HealCommand {
    ...
    public HealCommand(Hard hard) {
        this.hard = hard;
    }

    @PlayerExecute
    public void onPlayerExecute(Player p, String[] args) {
        if (!p.hasPermission(hard.getStorage().permissions.get("heal"))) {
        ...
        if (args.length == 0) {
            ...
            p.sendMessage(StringUtil.inColor("&c&lHARD &7>> &fВы восстановили себе здоровье!"));
            return;
        }

        if (!p.hasPermission(hard.getStorage().permissions.get("heal_other"))) {
            DateMessage.NO_PERMISSION.sendMessage(p);
            return;
        }
        ...
        p.sendMessage(StringUtil.inColor("&c&lHARD &7>> &fВы восстановили игроку: &c" + args[0] + " &fздоровье!"));
    }
    ...
    public List<String> onTabExecute(CommandSender sender, String[] args) {
        if (!sender.hasPermission(hard.getStorage().permissions.get("heal_other")))
